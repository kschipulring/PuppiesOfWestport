    ...
    protected function _addCategories($importData, $store)
    {
		...
		foreach($categories_update as $cat_data){
			...
			if($cat_path_name_final == $final_cat){
				$category_ids = end($cat_path_data);  
				break;
			}
		...
		if($category_id != ""){
				...
				$dublicate_catIds = $category_id;
				$update_cat = Mage::getModel('catalog/category')->setStoreId($store_id)->load($dublicate_catIds);
				...
				for($e=0; $e<count($product_data); $e++){  
					$product_id = Mage::getModel("catalog/product")->getIdBySku($product_data[$e]);
 					if($product_id != "") {
						...
						if(!in_array($dublicate_catIds, $origCats)) {
							$newCategories = array_merge($origCats, array($dublicate_catIds));
